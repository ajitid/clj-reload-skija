  ...
  "2D rubber band effect for boundary resistance.
   ...
   Usage:
     (rubber-band-clamp-2d
       (v/vec2 1700 2500)                 ;; current position (past bounds)
       {:min-x 0 :max-x 1600              ;; content scrolls 0-1600 in X
        :min-y 0 :max-y 2400}             ;; content scrolls 0-2400 in Y
       {:coeff 0.55 :dims (v/vec2 400 600)})  ;; viewport is 400x600
...
(defn rubber-band-clamp-2d
  "Clamp a 2D point within rectangular bounds, applying rubber band effect outside.
   ...
   Returns Vec2[clamped-x clamped-y]"
  [point {:keys [min-x max-x min-y max-y]} {:keys [coeff dims]}]
  (let [[px py] point
        [dim-x dim-y] dims]
    (v/vec2 (rb/rubber-band-clamp px min-x max-x {:coeff coeff :dim dim-x})
